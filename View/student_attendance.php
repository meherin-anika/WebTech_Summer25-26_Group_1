...
// If a course is selected, fetch attendance records immediately
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: Arial, sans-serif; }
        ...
        .select-group { margin-bottom: 25px; }
        .select-group label { display: block; margin-bottom: 8px; font-size: 14px; font-weight: bold; color: #741f2b; }
        .select-group select { width: 100%; padding: 11px; border: 1px solid #d8cdb8; border-radius: 6px; background: white; color: #000000; font-size: 14px; outline: none; }
        .select-group select:focus { border-color: #741f2b; }

        .text-list { display: flex; flex-direction: column; gap: 12px; }
        .text-row { display: flex; justify-content: space-between; padding: 12px 0; border-bottom: 1px solid #eadfc9; font-size: 14px; }
    ...
    <div class="text-list">
        <?php if (!empty($selected_course)): ?>
            <?php if ($attendance_records && $attendance_records->num_rows > 0): ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <p class="empty-msg">No attendance records found for this course.</p>
            <?php endif; ?>
        <?php else: ?>
            <p class="empty-msg">Please select a course above to view your attendance.</p>
        <?php endif; ?>
    </div>
</div>
