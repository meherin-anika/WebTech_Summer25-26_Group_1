...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: Arial, sans-serif; }
        ...
        .grade-highlight { color: #741f2b; font-weight: bold; }
...
<div class="container">
    ...
    <div class="text-list">
        <?php if ($marks_records && $marks_records->num_rows > 0): ?>
            <?php while ($row = $marks_records->fetch_assoc()): ?>
                <div class="item-block">
                    <p><strong>Course:</strong> <?php echo htmlspecialchars($row['course_code'] . " - " . $row['course_name']); ?> (ID: <?php echo htmlspecialchars($row['course_id']); ?>)</p>
                    <p><strong>Credits:</strong> <?php echo htmlspecialchars($row['credit']); ?> | <strong>Marks:</strong> <?php echo ($row['marks'] !== null) ? htmlspecialchars($row['marks']) : "N/A"; ?> | <strong>Grade:</strong> <span class="grade-highlight"><?php echo ($row['grade'] !== null) ? htmlspecialchars($row['grade']) : "N/A"; ?></span></p>
                </div>
            <?php endwhile; ?>
        <?php else: ?>
            <p class="empty-msg">No enrolled courses or marks available.</p>
        <?php endif; ?>
    </div>
</div>
